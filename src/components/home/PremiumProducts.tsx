...
async function getPremiumProducts() {
    try {
        ...
        console.log(`📦 Fetched ${products.length} total products for Premium Products`);

        // Filter products that have the 'premium' tag
        const premiumProducts = products.filter((p: any) =>
            p.tags && Array.isArray(p.tags) && p.tags.includes('premium')
        );

        console.log(`💎 Found ${premiumProducts.length} premium products`);

        // Randomly shuffle and take 8 products — fall back to the
        // highest-priced products when nothing is tagged 'premium' yet.
        const selected = premiumProducts.length > 0
            ? premiumProducts.sort(() => 0.5 - Math.random()).slice(0, 8)
            : [...products].sort((a: any, b: any) => (b.price || 0) - (a.price || 0)).slice(0, 8);

        console.log(`✅ Selected ${selected.length} random premium products`);
...
export const PremiumProducts = async () => {
            ...
            <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-white">
                <div className="mb-12">
                    ...
                    <p className="text-slate-600 text-lg">
                        Luxury and premium quality collections
                    </p>
                </div>
                <div className="text-center py-12 bg-slate-50 rounded-2xl">
                    <p className="text-slate-600">No premium products available yet. Add products with the &apos;premium&apos; tag to see them here.</p>
                </div>
            </section>
        );
    }
