...
async function getClassicProducts() {
    try {
        // Fetch all products
        ...
        console.log(`📦 Fetched ${products.length} total products for Classic Products`);

        // Filter products that have the 'classic' tag
        const classicProducts = products.filter((p: any) =>
            p.tags && Array.isArray(p.tags) && p.tags.includes('classic')
        );
...
export const ClassicProducts = async () => {
    const products = await getClassicProducts();
        ...
        return (
            <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-white">
                ...
                <div className="text-center py-12 bg-slate-50 rounded-2xl">
                    <p className="text-slate-600">No classic products available yet. Add products with the &apos;classic&apos; tag to see them here.</p>
                </div>
            </section>
        );
    }
