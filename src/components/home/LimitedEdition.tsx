...
async function getLimitedEditionProducts() {
    try {
        ...
        console.log(`📦 Fetched ${products.length} total products for Limited Edition Products`);

        // Filter products that have the 'limited' tag
        const limitedEditionProducts = products.filter((p: any) =>
            p.tags && Array.isArray(p.tags) && p.tags.includes('limited')
        );
...
export const LimitedEdition = async () => {
        ...
        return (
            <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-white">
                ...
                <div className="text-center py-12 bg-slate-50 rounded-2xl">
                    <p className="text-slate-600">No limited edition products available yet. Add products with the &apos;limited&apos; tag to see them here.</p>
                </div>
            </section>
        );
    }
    ...
    return (
        <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-white">
            ...
            <div className="mt-8 text-center md:hidden">
                ...
                >
                    View All Limited Edition Products
                </Link>
            </div>
        </section>
    );
